...
def getTotalVolume(OnCubes,  R):
  v = 0
  for switch_cube, n in OnCubes.items():
    v += n * R[switch_cube].getVolume()
  return v

def solve(switches_and_cubes):

  OnCubes = Counter()
  # es wurde "leider" eine eigene Klasse Cube erstellt.
  # verschiedene Objekte mit gleichen Koordinaten sind verschiedene Objekte - und werden im Counter damit (exponentiell) mehrfach gezählt.
  # daher wird als Schlüssel nicht das Objekt - sonder ein Hash - die Ausgabe von __str__()  - verwendet.
  # die Regestry R dient als  "*Übersetzer" vom Hash zum Cube-Objekt
  R = defaultdict()  # hash -> Cube (hash:= Cube.__str__())
  ...
  for switch_cube in switches_and_cubes:
    ...
    for onCube in OnCubes:
      ...
      assert intersection.__str__() == current_cube.intersection(onCube).__str__()
      onCubes_temp[intersection.__str__()] -= OnCubes[onCube.__str__()]
      R[intersection.__str__()] = intersection
    ...
    if current_switch:
        onCubes_temp[current_cube.__str__()] += 1
        R[current_cube.__str__()] = current_cube
        #print("add curent cube to dict, V:", current_cube.getVolume(), " V_total:", getTotalVolume(OnCubes,  R))

    for c in onCubes_temp:
        OnCubes[c.__str__()] += onCubes_temp[c.__str__()]
        R[current_cube.__str__()] = current_cube



  return getTotalVolume(OnCubes, R)
...
def test():
  ...
  for c in c_no_i:
        assert(c.intersection(c_i).__str__()  == c_i.__str__())
